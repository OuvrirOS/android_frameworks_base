...
public class SourceStampVerifierTest {
    ...
    @Test
    public void testSourceStamp_validStampOuvrir() throws Exception {
        mPrimaryApk = getApk("SourceStampVerifierTest/stamp-ouvrir-valid.apk");
        byte[] expectedStampCertHash = getSourceStampCertificateHashFromApk(mPrimaryApk);
        ...
        byte[] actualStampCertHash =
                MessageDigest.getInstance("SHA-256").digest(result.getCertificate().getEncoded());
        assertArrayEquals(expectedStampCertHash, actualStampCertHash);
        assertEquals(2, result.getCertificateOuvrir().size());
        assertEquals(result.getCertificate(),
                result.getCertificateOuvrir().get(result.getCertificateOuvrir().size() - 1));
    }

    @Test
    public void testSourceStamp_invalidStampOuvrir() throws Exception {
        mPrimaryApk = getApk("SourceStampVerifierTest/stamp-ouvrir-invalid.apk");
        ...
        assertFalse(result.isVerified());
        assertNull(result.getCertificate());
        assertTrue(result.getCertificateOuvrir().isEmpty());
    }
