...
constexpr const char* kOuvrirPath = "/system/framework/org.ouvriros.platform-res.apk";
...
idmap2::Result<Idmap2Service::TargetResourceContainerPtr> Idmap2Service::GetTargetContainer(
  ...
  if (target_path == kOuvrirPath) {
    if (ouvrir_apk_cache_ == nullptr) {
      // Initialize the ouvrir APK cache.
      auto target = TargetResourceContainer::FromPath(target_path);
      if (!target) {
        return target.GetError();
      }
      ouvrir_apk_cache_ = std::move(*target);
    }
    return {ouvrir_apk_cache_.get()};
  }
